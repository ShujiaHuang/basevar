...
def create_single_batchfile(chrid, bigstart, bigend, regions, batch_align_files, fa, mapq,
    ...
    with utils.Open(out_batch_file, "wb", isbgz=True) if out_batch_file.endswith(".gz") else \
        ...
        OUT.write(b"##fileformat=BaseVarBatchFile_v1.0\n")
        if batch_sample_ids:
            OUT.write(bytes("##SampleIDs=%s\n" % ",".join(batch_sample_ids), "utf-8"))

        if not justbase:
            suff_header = ["#CHROM", "POS", "REF", "Depth(CoveredSample)", "MappingQuality",
