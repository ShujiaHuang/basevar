...
def load_target_position(referencefile, posfile, region_info):
    ...
    if not regions:
        ...
        regions = [[ci, 1, fa.get_reference_length(ci)] for ci in fa.references]
...
class FileForQueueing(object):
    ...
    def __init__(self, the_file, line, is_del_raw_file=False):
        ...
        chrom = cols[0].decode("utf-8") # convert 'bytes' to 'string'
...
def merge_files(temp_file_names, final_file_name, output_isbgz=False, is_del_raw_file=False):
    ...
    for index, file_name in enumerate(temp_file_names):
        ...
        for line in the_file:
            ...
            if line.startswith(b"#"):
                ...
                heapq.heappush(the_heap, (index, the_file_for_queueing))
                break

        # If there are no calls in the temp file, we still want to remove it.
        else:
            the_file.close()
            if is_del_raw_file:
                os.remove(file_name)
    ...
    while len(the_heap) != 0:
        ...
        i, next_file = heapq.heappop(the_heap)
        output_file.write(next_file.line)

        # Put file back on heap
        try:
            next(next_file)
            heapq.heappush(the_heap, (i, next_file))
        except StopIteration:
            continue
...
def merge_batch_files(temp_file_names, final_file_name, output_isbgz=False, is_del_raw_file=False, justbase=False):
    ...
    while not eof:
        ...
        if eof:
            ...
            if is_error:
                sys.stderr.write("[ERROR] %s\n[ERROR]Error happen when 'merge_batch_files', they don't match the same "
                                 "positions in above files.\n" % "\n".join(temp_file_names))
            continue

        if sampleinfos[0].startswith(b"#"):
            # Header line
            if sampleinfos[0].startswith(b"##SampleIDs="):
                sampleid_info = [s.split(b"=")[1] for s in sampleinfos]
                output_file.write(b"##SampleIDs=%s\n" % b",".join(sampleid_info))
            else:
            ...
            depth = bytes(str(depth), "utf-8")
            if not justbase:
                mapqs = b",".join(mapqs)
                read_bases = b",".join(read_bases)
                read_base_quals = b",".join(read_base_quals)
                read_pos_rank = b",".join(read_pos_rank)
                strands = b",".join(strands)

                # [CHROM POS REF Depth MappingQuality Readbases ReadbasesQuality ReadPositionRank Strand]
                output_file.write(b"%s\n" % b"\t".join([chrid, position, ref_base, depth, mapqs, read_bases,
                                                        read_base_quals, read_pos_rank, strands]))
            else:
                # [CHROM POS REF Depth Readbases]
                output_file.write(b"%s\n" % b"\t".join([chrid, position, ref_base, depth, ",".join(read_bases)]))
...
def load_popgroup_info(samples, in_popgroup_file):
    ...
    with Open(in_popgroup_file, 'rb') as f:
        ...
        for line in f:
            ...
            tmpdict[sample_id] = group_id + b'_AF'
