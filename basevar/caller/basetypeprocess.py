...
def _fetch_baseinfo_by_position_from_batchfiles(chrid, position, ref_base, infolines):
    ...
    for i, col in enumerate(infolines):
        ...
        for j in range(len(col)):
            if type(col[j]) is bytes:
                col[j] = col[j].decode('utf-8')

        col[1], col[3] = int(col[1]), int(col[3])
    ...
    mapqs = list(map(int, ",".join(mapqs).split(",")))
    sample_bases = ",".join(sample_bases).split(",")
    sample_base_quals = list(map(int, ",".join(sample_base_quals).split(",")))
    read_pos_rank = list(map(int, ",".join(read_pos_rank).split(",")))
...
def _base_depth_and_indel(bases):
    ...
    indel_string = ','.join([k + '|' + str(v) for k, v in indel_depth.items()]) if indel_depth else "."
...
def _out_vcf_line(chrid, position, ref_base, bases, mapqs, read_pos_rank, sample_base_qual,
    ...
    out_file_handle.write('\t'.join([chrid, str(position), '.', ref_base,
                                     ...
                                     ';'.join([k + '=' + v for k, v in sorted(info.items(), key=lambda x: x[0])]), 
                                     'GT:AB:SO:BP'] + samples) + '\n')
    return
